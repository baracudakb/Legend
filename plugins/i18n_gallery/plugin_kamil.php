<?php
i18n_gallery_register('kamil', 'kamil', 
  '<strong>kamil</strong> is a slideshow plugin that supports many different types of transition effects.<br/>'.
  'License: MIT and GPL<br/>'.
  '<a target="_blank" href="http://jquery.malsup.com/cycle/">http://jquery.malsup.com/cycle/</a>',
  'i18n_gallery_kamil_edit', 'i18n_gallery_kamil_header', 'i18n_gallery_kamil_content');

function i18n_gallery_kamil_edit($gallery) {
?>
  <p>
    <label for="kamil-width"><?php i18n('i18n_gallery/MAX_DIMENSIONS'); ?></label>
    <input type="text" class="text" id="kamil-width" name="kamil-width" value="<?php echo @$gallery['width']; ?>" style="width:5em"/>
    x
    <input type="text" class="text" id="kamil-height" name="kamil-height" value="<?php echo @$gallery['height']; ?>" style="width:5em"/>
    &nbsp;
    <span id="kamil-crop-span">
      <input type="checkbox" id="kamil-crop" name="kamil-crop" value="1" <?php echo @$gallery['crop'] ? 'checked="checked"' : ''; ?> style="vertical-align:middle"/> 
      <?php i18n('i18n_gallery/CROP'); ?>
    </span>
  </p>
  <script type="text/javascript">
    function changekamilSize() {
        var show = $.trim($('#kamil-width').val()) != '' && $.trim($('#kamil-height').val()) != '';
        if (show) $('#kamil-crop-span').show(); else $('#kamil-crop-span').hide().find('input').attr('checked',false);
    }
    $(function() {
      $('#kamil-width, #kamil-height').change(changekamilSize);
      changekamilSize();
    });
  </script>
  <p>
    <label for="kamil-textpos"><?php i18n('i18n_gallery/TEXT_POSITION'); ?></label>
    <select class="text" name="kamil-textpos">
  ...
  <p>
    <label for="kamil-textwidth"><?php i18n('i18n_gallery/TEXT_WIDTH'); ?></label>
    <input type="text" class="text" id="kamil-textwidth" name="kamil-textwidth" value="<?php echo @$gallery['textwidth']; ?>" style="width:5em"/>
  </p>
  <p>
    <label for="kamil-interval"><?php i18n('i18n_gallery/INTERVAL'); ?></label>
    <input type="text" class="text" id="kamil-interval" name="kamil-interval" value="<?php echo @$gallery['interval']; ?>" style="width:5em"/>
  </p>
  <p>
    <label for="kamil-effect"><?php i18n('i18n_gallery/EFFECT'); ?></label>
    <select class="text" name="kamil-effect">
  ...
  <p>
  <p>
    <label for="kamil-navtype"><?php i18n('i18n_gallery/NAVIGATION_TYPE'); ?></label>
    <select class="text" name="kamil-navtype">
...
function i18n_gallery_kamil_header($gallery) {
  global $SITEURL;
...
function i18n_gallery_kamil_content($gallery, $pic) {
  global $SITEURL;
  $id = i18n_gallery_id($gallery);
  $w = @$gallery['width'] ? $gallery['width'] : (@$gallery['height'] ? (int) $gallery['height']*$gallery['items'][0]['width']/$gallery['items'][0]['height'] : $gallery['items'][0]['width']);
  $h = @$gallery['height'] ? $gallery['height'] : (@$gallery['width'] ? (int) $gallery['width']*$gallery['items'][0]['height']/$gallery['items'][0]['width'] : $gallery['items'][0]['height']);
